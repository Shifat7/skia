...
local, pre-PR comprehension checkpoint for one developer: a small source-backed
behavior check, one prediction, and a visible record of anything it could not
analyze. The shell in [`src/main.ts`](src/main.ts) is unimplemented
...
for this decision. Labeled transcripts are in
...
The other reading examples below are **Illustrative / intended UX — not a live CLI session**.
...
Skia is an early local checkpoint. The longer pages are
[status](https://shifat7.github.io/skia/status/) and
[privacy](https://shifat7.github.io/skia/privacy/).
...
- The intended job is a diff- and snapshot-oriented reading for one developer.
  CodeRabbit, Greptile, and Copilot Review remain hosted PR tools. This README
  states no catch rate against them.
...
- Phase 0 has no product telemetry and no Skia review backend. A future
  repository mode can send bounded context only to an endpoint you configure,
  after consent. A cloud model means the prompt leaves the machine.
- Public stars are not a quality signal. Use the foundation tests and the
  labeled samples in [`examples/illustrative/`](examples/illustrative/).
- npm is the first install channel, and the package is still private. Homebrew
  and a curl installer have no commands until they work. Node.js 24.x
  (`>=24.0.0 <25`) is the engine floor.

## Run the repository today

These commands need Node.js 24.x and npm. They check the foundation. They do
not install a review CLI.
