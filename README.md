...
- runnable `skia review` support for exactly one staged `.ts` file containing
  one named function with a strict literal guard and literal return.

The pilot emits one deterministic relation, asks for one JSON-scalar prediction
or skip, persists the response before source-derived feedback, and writes a
validated local receipt. Prediction input is limited to 4,096 UTF-8 bytes;
enter the bare word `skip` to skip, or JSON `"skip"` to predict that string.
Other TypeScript shapes, TSX/Python reduction,
...
These commands verify the foundation and narrow staged-review pilot. They do
not run the future repository-review workflow.
...
| Term | Meaning today |
|---|---|
| `source-derived relation evidence` | Implemented for one staged TypeScript literal guard and return. `collapsed equivalence evidence` remains unimplemented. |
...
| What does the staged-review CLI currently do? | [`src/main.ts`](src/main.ts) and [`src/staged/`](src/staged/) |
...
Do not broaden the pilot grammar or start the repository scanner, agent
transport, HLD/LLD generation, or behavioral study unless that work is
explicitly scoped by a maintainer. Read
